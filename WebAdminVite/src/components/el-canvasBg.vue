<script setup lang="ts">
    nextTick(() => {
        let canvas = <HTMLCanvasElement | null>document.getElementById('canvasBg');
        let w = window.innerWidth,
            h = window.innerHeight;
        if (canvas != null) {
            const ctx = canvas.getContext('2d'),
                opts = {
                    len: 20, // 前進格數&六角路徑大小 (基礎為20)
                    count: 10, // 線條數量，數字越小移動越快(可能和運算數度有關)(基礎50)
                    baseTime: 5, // 刷新時間? (數字越小 跑越快)
                    addedTime: 10,
